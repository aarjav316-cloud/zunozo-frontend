...
const EventCard = ({ event }) => {
  ...
  const formattedMobileDate = `${weekday}, ${day} ${month}`;

  return (
    <>
      {/* ========================================================
          1. MOBILE EVENT CARD (block sm:hidden)
          Clean full-bleed poster image + Info below image (BookMyShow style)
         ======================================================== */}
      <div
        onClick={handleClick}
        className="block sm:hidden group cursor-pointer w-full"
      >
        {/* Full-Bleed Poster Image */}
        <div className="relative aspect-[3/4] w-full rounded-2xl overflow-hidden mb-2.5 bg-zinc-900">
          {event.coverImage ? (
            <img
          ...
          <button
            ...
            className="absolute top-2.5 right-2.5 z-10 backdrop-blur-md bg-black/40 border border-white/10 rounded-full w-7 h-7 flex items-center justify-center transition-transform active:scale-95 shadow-md"
        ...
        <div className="space-y-1 px-0.5">
          {/* Date Badge Pill */}
          <div>
            <span className="inline-block px-2.5 py-0.5 bg-zinc-800/90 rounded-md text-[11px] font-medium text-zinc-300">
              {formattedMobileDate}
            </span>
          </div>

          {/* Event Title */}
          <h3 className="text-sm font-bold text-white leading-snug line-clamp-2 tracking-tight group-hover:text-zinc-200">
            {event.title}
          </h3>

          {/* Location */}
          {event.venue?.city && (
            <p className="text-[12px] text-zinc-400 truncate flex items-center gap-1 font-medium">
              <svg
          ...
          <div className="flex items-center justify-between pt-0.5">
            ...
            {event.category && (
              <span className="text-[10px] font-semibold text-zinc-400 bg-zinc-900 border border-zinc-800 px-2 py-0.5 rounded-full uppercase tracking-wide">
                {event.category}
              </span>
            )}
          </div>
      ...
      {/* ========================================================
          2. DESKTOP EVENT CARD (hidden sm:block)
          Preserved 100% untouched for desktop/tablet
         ======================================================== */}
      <div
        onClick={handleClick}
        className="hidden sm:block flex-none w-80 group cursor-pointer"
      >
